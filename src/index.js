...
const Book = (props) => {
  const { title, author, image, getParaghraph, children } = props;

  return (
    <article className="book">
      <img src={image} alt={title} />
      <h2>{title}</h2>
      <button onClick={getParaghraph}>Show Title</button>
...
const BookList = () => {
  const paragraph = "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Necessitatibus, adipisci.";

  const getParaghraph = () => console.log(paragraph);
  ...
  return (
    <section className="booklist">
      {newBooks.map((book) => {
        return <Book {...book} getParaghraph={getParaghraph} key={book.id} />;
      })}
    </section>
  );
};
